        ...
        <div className="cart-content">
                    ...
                    <td style={{ padding: '15px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        <img
                          ...
                          alt={item.title}
                          style={{ width: '55px', height: '75px', objectFit: 'cover', borderRadius: '4px', backgroundColor: '#eee' }}
                          onError={(e) => { e.target.style.backgroundColor = '#ccc'; e.target.src = 'data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs='; }}
                        />
                        <div>
                          <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>{item.title || '제목 없음'}</div>
                          <div style={{ fontSize: '13px', color: '#777' }}>{item.author || '저자 미상'}</div>
                          {price > 0 && <div style={{ fontSize: '13px', color: '#e53e3e', marginTop: '4px' }}>{formatPrice(price)}</div>}
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '15px', textAlign: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                        <button
                          ...
                          style={{ width: '28px', height: '28px', border: '1px solid #ddd', borderRadius: '4px', cursor: 'pointer', backgroundColor: '#fff' }}
                        >−</button>
          ...
          <div style={{ marginTop: '30px', textAlign: 'right', borderTop: '2px solid #333', paddingTop: '20px' }}>
            <p style={{ fontSize: '14px', color: '#777', marginBottom: '6px' }}>
              총 {items.reduce((sum, i) => sum + i.quantity, 0)}권
            </p>
            ...
            <button
              type="button"
              onClick={onContinueShopping}
              style={{ marginRight: '10px', padding: '12px 24px', backgroundColor: '#fff', border: '1px solid #0d6efd', color: '#0d6efd', borderRadius: '4px', cursor: 'pointer' }}
            >
              쇼핑 계속하기
            </button>
            <button
              ...
              style={{ padding: '12px 24px', backgroundColor: '#0d6efd', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer', fontWeight: 'bold' }}
            >
              주문하기
            </button>
          </div>
